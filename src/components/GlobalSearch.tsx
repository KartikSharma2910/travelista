...
import { Compass, Globe, MapPin, Search, Users, X } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      try {
        ...
        const [{ data: exps }, { data: trips }] = await Promise.all([
          supabase
            .from("experiences")
            .select("id, title, category, location")
            .or(
              `title.ilike.%${query}%,category.ilike.%${query}%,location.ilike.%${query}%`
            )
            .eq("status", "approved")
            .limit(4),
        ...
        trips?.forEach((t) => {
          items.push({
            ...
            id: t.id,
            title: t.title,
            subtitle: `${t.destination || "Various"} · ${t.trip_type?.replace(
              "_",
              " "
            )}`,
            icon: Compass,
  ...
  const handleSelect = (r: SearchResult) => {
    ...
    switch (r.type) {
      case "host":
        ...
        break;
      case "experience":
        navigate(`/experience/${r.id}`);
        break;
      case "trip":
        navigate(`/trip/${r.id}`);
        break;
      case "destination":
        navigate(`/destination/${r.id}`);
        break;
    }
  };
