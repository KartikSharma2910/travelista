...
const VirtualTour = ({ siteName, siteType, images }: VirtualTourProps) => {
  ...
  const next = () => {
    setCurrentIndex((currentIndex + 1) % tourImgs.length);
    setZoom(1);
    setRotation(0);
  };
  const prev = () => {
    setCurrentIndex((currentIndex - 1 + tourImgs.length) % tourImgs.length);
    setZoom(1);
    setRotation(0);
  };
            ...
            >
              <div className="absolute top-0 left-0 right-0 z-10 flex items-center justify-between p-4 bg-gradient-to-b from-foreground/50 to-transparent">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => setIsOpen(false)}
                    className="w-8 h-8 rounded-full bg-primary-foreground/10 backdrop-blur-sm flex items-center justify-center text-primary-foreground hover:bg-primary-foreground/20"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ...
              <button
                onClick={prev}
                className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-primary-foreground/10 backdrop-blur-sm flex items-center justify-center text-primary-foreground hover:bg-primary-foreground/20"
              >
                <ChevronLeft className="w-5 h-5" />
              </button>
              <button
                onClick={next}
                className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-primary-foreground/10 backdrop-blur-sm flex items-center justify-center text-primary-foreground hover:bg-primary-foreground/20"
              >
              ...
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-foreground/60 backdrop-blur-md rounded-full px-4 py-2">
                <button
                  ...
                  className="w-8 h-8 rounded-full hover:bg-primary-foreground/10 flex items-center justify-center text-primary-foreground"
                >
                  <ZoomOut className="w-4 h-4" />
                </button>
                <span className="text-xs text-primary-foreground/80 min-w-[40px] text-center">
                  {Math.round(zoom * 100)}%
                </span>
                <button
