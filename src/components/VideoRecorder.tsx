...
import { Check, Circle, Square, Upload, Video, X } from "lucide-react";
...
const VideoRecorder = ({
  ...
  const [mode, setMode] = useState<"choose" | "record" | "upload" | "preview">(
    "choose"
  );
  ...
  const startCamera = useCallback(async () => {
    try {
      ...
      setError(
        "Camera access denied. Please allow camera permissions or upload a video file instead."
      );
    }
  }, []);
  ...
  const confirmVideo = useCallback(() => {
    if (videoUrl)
      onVideoReady(videoUrl, mode === "record" ? "recorded" : "uploaded");
