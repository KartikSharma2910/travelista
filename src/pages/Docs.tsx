      ...
      <div className="pt-24 pb-16 px-4 sm:px-6 lg:px-8 mx-auto max-w-7xl">
          ...
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm">
                Travelista — Complete technical reference
              </p>
            </div>
          </div>
        </motion.div>
        ...
        <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div
              key={s.label}
              className="rounded-lg bg-card p-3 shadow-card flex items-center gap-3"
            >
                ...
                {current.content.split("\n").map((line, i) => {
                  ...
                  if (line.startsWith("- [x]")) {
                    return (
                      <p
                        key={i}
                        className="text-sm flex items-center gap-2 text-accent ml-4"
                      >
                        <span>✅</span> {line.replace("- [x] ", "")}
                      </p>
                    );
                  }
                  if (line.startsWith("- [ ]")) {
                    return (
                      <p
                        key={i}
                        className="text-sm flex items-center gap-2 text-muted-foreground ml-4"
                      >
                  ...
                  if (line.startsWith("|")) {
                    ...
                    return (
                      <div
                        key={i}
                        className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs mb-1 ml-4"
                      >
                        {cells.map((c, j) => (
                          <span
                            key={j}
                  ...
                  const formatted = line
                    .replace(
                      /`([^`]+)`/g,
                      '<code class="bg-secondary text-primary px-1 py-0.5 rounded text-xs font-mono">$1</code>'
                    )
                    .replace(
                      /\*\*(.+?)\*\*/g,
