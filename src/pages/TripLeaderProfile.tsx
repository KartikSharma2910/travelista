...
const TripLeaderProfile = () => {
  ...
  const avgRating = (
    pastTripsMock.reduce((s, t) => s + t.rating, 0) / pastTripsMock.length
  ).toFixed(1);
  const leaderScore = Math.round(
    Number(avgRating) * 20 + pastTripsMock.length * 5 + totalTravelers
  );
        ...
        >
              ...
              <div className="text-center sm:text-left flex-1">
                <div className="flex items-center gap-2 justify-center sm:justify-start flex-wrap">
                  <h1 className="text-2xl sm:text-3xl font-bold text-foreground">
                    {profile?.first_name || "Trip Leader"}{" "}
                    {profile?.last_name || ""}
                  </h1>
                  <span className="text-xs font-bold px-3 py-1 rounded-full bg-primary text-primary-foreground flex items-center gap-1">
                    <Compass className="w-3 h-3" /> Trip Leader
                  </span>
                </div>
                <p className="text-muted-foreground mt-1">
                  {profile?.bio || "Leading adventures across India"}
                </p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mt-6">
            {[
              {
                label: "Trips Led",
                value: pastTripsMock.length + trips.length,
                icon: Compass,
              },
          ...
          <div className="mt-8">
            ...
            <div className="flex gap-3 overflow-x-auto pb-2">
              {badges.map((b) => (
                <div
                  key={b.name}
                  className="shrink-0 w-32 rounded-xl bg-card p-4 shadow-card text-center hover:shadow-elevated transition-shadow"
                >
          ...
          <div className="mt-8">
            <h2 className="text-lg font-bold text-foreground mb-4 flex items-center gap-2">
              <Camera className="w-5 h-5 text-primary" /> Top Highlights
            </h2>
            <div className="flex gap-3 overflow-x-auto pb-2">
              {highlights.map((h) => (
                <motion.div
                ...
                >
                  ...
                  <p className="text-[10px] font-bold text-foreground text-center px-1 leading-tight">
                    {h.name}
                  </p>
                </motion.div>
              ))}
          ...
          <div className="mt-8">
            <h2 className="text-lg font-bold text-foreground mb-4 flex items-center gap-2">
              <Clock className="w-5 h-5 text-primary" /> Past Trips
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {pastTripsMock.map((t, i) => (
                <motion.div
                  key={i}
                  whileHover={{ y: -4 }}
                  className="rounded-xl bg-card shadow-card overflow-hidden group"
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <span className="text-xs text-muted-foreground flex items-center gap-1">
                        <Users className="w-3 h-3" /> {t.travelers} joined
                      </span>
                    </div>
                  </div>
                </motion.div>
          ...
          {trips.length > 0 && (
            <div className="mt-8">
              <h2 className="text-lg font-bold text-foreground mb-4 flex items-center gap-2">
                <TrendingUp className="w-5 h-5 text-primary" /> Active Trips
              </h2>
                    ...
                    <div className="rounded-xl bg-card p-5 shadow-card hover:shadow-elevated transition-shadow flex justify-between items-center">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          {t.destination} · {format(t.total_price)} ·{" "}
                          {t.duration || "Flexible"}
                        </p>
                      </div>
              ...
              {[
                {
                  ...
                  country: "USA",
                  rating: 5,
                  text: "Incredible trip leader! Made Ladakh absolutely magical. Every detail was perfectly planned.",
                  date: "Feb 2026",
                },
                {
                  name: "Thomas K.",
                  country: "Germany",
                  rating: 5,
                  text: "Best road trip I've ever been on. The group was amazing and the leader knew every hidden gem.",
                  date: "Dec 2025",
                },
                {
                  name: "Aiko T.",
                  country: "Japan",
                  rating: 4,
                  text: "Great experience, well organized. Would love to join another trip!",
                  date: "Oct 2025",
                },
              ].map((r, i) => (
