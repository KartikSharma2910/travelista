...
const roomFields: FieldConfig[] = [
  { key: "name", label: "Room Name", required: true },
  {
    ...
    label: "Room Type",
    type: "select",
    options: ["Private Room", "Shared Room", "Entire Home", "Heritage Suite"],
    required: true,
  },
...
const HostDashboard = () => {
  ...
  const [customExperiences, setCustomExperiences] = useLocalStorage<any[]>(
    "host_custom_experiences",
    []
  );
  const [customVehicles, setCustomVehicles] = useLocalStorage<any[]>(
    "host_custom_vehicles",
    []
  );
  const [customDishes, setCustomDishes] = useLocalStorage<any[]>(
    "host_custom_dishes",
    host.foodInfo?.dishes || []
  );
  const [customProperties, setCustomProperties] = useLocalStorage<any[]>(
    "host_custom_properties",
    host.stayInfo ? [host.stayInfo] : []
  );
  const [customRooms, setCustomRooms] = useLocalStorage<any[]>(
    "host_custom_rooms",
    host.stayInfo?.rooms || []
  );
  ...
  const submitExperienceRequest = async () => {
    if (!user || !expForm.title || !expForm.location) {
    ...
    const { error } = await supabase.from("experience_requests").insert({
      ...
      max_guests: expForm.maxGuests,
      is_year_round: expForm.isYearRound,
      ...
      vehicle_type: expForm.vehicleType || null,
      destination: expForm.destination || null,
      sub_category: expForm.subCategory || null,
      highlights: expForm.highlights
        ? expForm.highlights.split(",").map((s) => s.trim())
        : [],
      includes: expForm.includes
        ? expForm.includes.split(",").map((s) => s.trim())
        : [],
    });
    ...
    toast({
      ...
      description: "Admin will review and approve.",
    });
    setExpForm({
      title: "",
      description: "",
      ...
      <div className="pt-24 pb-16 px-4 sm:px-6 lg:px-8 mx-auto max-w-6xl">
        ...
        >
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              {hostProfile.city} · Host since 2024
            </p>
          </div>
        </motion.div>
                        ...
                        {b.status === "pending" && (
                          <div className="flex gap-2 mt-1">
                            <Button
                              ...
                              className="rounded-full text-xs px-3 bg-accent text-accent-foreground hover:bg-accent/90"
                            >
                              Accept
                            </Button>
                            <Button
        ...
        {activeTab === "bookings" && (
          <div className="mt-6 space-y-3">
            <h2 className="text-xl font-bold text-foreground mb-4">
              All Bookings ({hostBookings.length})
            </h2>
              ...
              hostBookings.map((b) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      {b.start_date} → {b.end_date} ·{" "}
                      {(b.services || []).join(", ")} · {b.guests} guest(s)
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-foreground">
                    ...
                    {b.status === "pending" && (
                      <div className="flex gap-2 mt-2">
                        <Button
                          ...
                          className="rounded-full bg-accent text-accent-foreground text-xs px-3"
                        >
                          Accept
                        </Button>
                        <Button
                    ...
                    {(b.status === "confirmed" || b.status === "completed") && (
                      <Button
                        ...
                        className="rounded-full text-xs px-3 gap-1 mt-1"
                      >
        ...
        {activeTab === "experiences" && (
          <div className="mt-6 space-y-8">
            <div>
              <h2 className="text-xl font-bold text-foreground mb-4">
                Your Experiences ({allExperiences.length})
              </h2>
                ...
                {allExperiences.map((exp) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-muted-foreground">
                      ${exp.price} · {exp.duration} · {exp.category}
                    </p>
                  </div>
                ))}
              </div>
            ...
            <div className="rounded-2xl bg-card p-6 shadow-card">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Submit a new experience for admin approval. Include all details
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="e.g. Ladakh Bike Expedition"
                  />
                </div>
                <div>
                  <label className="text-sm font-medium text-foreground">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setExpForm((p) => ({ ...p, location: e.target.value }))
                    }
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setExpForm((p) => ({ ...p, subCategory: e.target.value }))
                    }
                    placeholder="e.g. Motorcycle Tour"
                  />
                </div>
                <div>
                  <label className="text-sm font-medium text-foreground">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setExpForm((p) => ({ ...p, duration: e.target.value }))
                    }
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setExpForm((p) => ({ ...p, difficulty: e.target.value }))
                    }
                  >
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setExpForm((p) => ({ ...p, destination: e.target.value }))
                    }
                  />
                </div>
                <div>
                  ...
                  <Input
                    className="mt-1"
                    value={expForm.vehicleType}
                    onChange={(e) =>
                      setExpForm((p) => ({ ...p, vehicleType: e.target.value }))
                    }
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setExpForm((p) => ({ ...p, highlights: e.target.value }))
                    }
                    placeholder="Khardung La, Pangong Lake"
                  />
                </div>
                <div>
                  <label className="text-sm font-medium text-foreground">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setExpForm((p) => ({ ...p, includes: e.target.value }))
                    }
                    placeholder="Bike rental, Meals, Permits"
                  />
                </div>
              </div>
              ...
              <div className="mt-4 rounded-lg bg-secondary/50 p-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm font-medium text-foreground">
                    Year-round availability (12 months open)
                  </span>
                </label>
                {!expForm.isYearRound && (
                  <div className="grid grid-cols-3 gap-3 mt-3">
                    ...
                    <div>
                      ...
                      <Input
                        type="date"
                        className="mt-1"
                        value={expForm.validTo}
                        onChange={(e) =>
                          setExpForm((p) => ({ ...p, validTo: e.target.value }))
                        }
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        type="date"
                        className="mt-1"
                        value={expForm.lastBookingDate}
                        onChange={(e) =>
                          setExpForm((p) => ({
                            ...p,
                            lastBookingDate: e.target.value,
                          }))
                        }
                      />
                    </div>
              ...
              <Button
                ...
                onClick={submitExperienceRequest}
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          {req.category} · {req.location} · ${req.price}
                        </p>
                      </div>
          ...
          <div className="mt-6 space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-bold text-foreground flex items-center gap-2">
                  <Home className="w-4 h-4 text-primary" /> Properties
                </h3>
                <Button
                  size="sm"
              ...
              {customProperties.map((prop, i) => (
                <div
                  key={i}
                  className="rounded-lg bg-card p-5 shadow-card mb-3"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
            ...
            <div>
              ...
              {allVehicles.map((v, i) => (
                <div
                  key={i}
                  className="rounded-lg bg-card p-4 shadow-card mb-2 flex justify-between items-center"
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {v.capacity} pax · ${v.pricePerDay}/day
                    </p>
                  </div>
                  <Button
                    variant="outline"
                    ...
                    onClick={() =>
                      setEditDialog({
                        ...
                        onSave: (d) => {
                          const ci =
                            i - (host.transportInfo?.vehicles?.length || 0);
                          if (ci >= 0)
                            setCustomVehicles((p) =>
                              p.map((x, j) => (j === ci ? { ...x, ...d } : x))
                            );
                          toast({ title: "Updated!" });
                        },
                      })
            ...
            {customDishes.map((dish, i) => (
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {dish.cuisine} · ${dish.price}/person
                  </p>
                </div>
                <Button
                  variant="outline"
                  ...
                  onClick={() =>
                    setEditDialog({
                      ...
                      onDelete: () => {
                        setCustomDishes((p) => p.filter((_, j) => j !== i));
                        toast({ title: "Removed" });
                      },
                    })
                  }
        ...
        {activeTab === "reviews" && (
          <div className="mt-6 space-y-4">
            <h2 className="text-xl font-bold text-foreground mb-4">
              Your Reviews ({hostReviews.length})
            </h2>
            {hostReviews.map((r) => (
        ...
        {activeTab === "invoices" && (
          <div className="mt-6">
            <h2 className="text-xl font-bold text-foreground mb-4">
              Invoices ({hostInvoices.length})
            </h2>
            {hostInvoices.length === 0 ? (
              <div className="text-center py-12">
                <Receipt className="w-12 h-12 text-muted-foreground/30 mx-auto mb-3" />
                <p className="text-muted-foreground">
                  No invoices yet. Generate invoices from confirmed bookings.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {hostInvoices.map((inv) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        {inv.currency} {inv.total_amount} (incl. tax{" "}
        ...
        {activeTab === "messages" && (
          <div className="mt-6">
            ...
            {["Sarah M. (USA)", "Thomas K. (Germany)", "Yuki T. (Japan)"].map(
              (name, i) => (
                <div
                  key={name}
                  className="rounded-lg bg-card p-4 shadow-card flex items-center gap-4 mb-3"
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Inquiry about {["tour", "stay", "wedding"][i]}
                    </p>
                  </div>
        ...
        {activeTab === "settings" && (
          <div className="mt-6 space-y-6 max-w-xl">
            ...
            <div className="rounded-lg bg-card p-5 shadow-card space-y-4">
              <h3 className="font-bold text-foreground flex items-center gap-2">
                <Settings className="w-4 h-4 text-primary" /> Profile
              </h3>
              <div className="space-y-3">
                <div>
                  <label className="text-sm font-medium text-foreground">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setHostProfile((p) => ({ ...p, name: e.target.value }))
                    }
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setHostProfile((p) => ({ ...p, city: e.target.value }))
                    }
                  />
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setHostProfile((p) => ({ ...p, bio: e.target.value }))
                    }
                  />
                </div>
                <div>
            ...
            <div className="rounded-lg bg-card p-5 shadow-card space-y-4">
              <h3 className="font-bold text-foreground flex items-center gap-2">
                <Globe className="w-4 h-4 text-primary" /> Social
              </h3>
              <div className="space-y-3">
                <div className="flex items-center gap-2">
