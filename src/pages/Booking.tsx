...
const serviceOptions = [
  {
    key: "Guide",
    label: "Local Guide",
    desc: "Personalized tours & experiences",
    icon: "🧭",
    price: 40,
  },
  {
    key: "Stay",
    label: "Homestay",
    desc: "Authentic local accommodation",
    icon: "🏡",
    price: 35,
  },
  {
    key: "Transport",
    label: "Transport",
    desc: "Vehicle with host as driver",
    icon: "🚗",
    price: 50,
  },
  {
    key: "Food",
    label: "Food & Dining",
    desc: "Home-cooked meals & food experiences",
    icon: "🍛",
    price: 20,
  },
];
...
const Booking = () => {
  ...
  const handleSubmit = async () => {
    ...
    toast({
      title: "Booking Sent! 🎉",
      description: `Your booking has been sent to ${host.name}.`,
    });
  };
        ...
        <div className="pt-24 pb-16 px-4 sm:px-6 lg:px-8 mx-auto max-w-2xl text-center">
          ...
          >
            ...
            <p className="mt-3 text-muted-foreground">
              Your request has been sent to <strong>{host.name}</strong>.
            </p>
            <div className="mt-6 p-4 rounded-xl bg-secondary text-left space-y-2">
              ...
              {startDate && endDate && (
                <p className="text-sm">
                  <strong>Dates:</strong> {format(startDate, "PPP")} –{" "}
                  {format(endDate, "PPP")}
                </p>
              )}
              <p className="text-sm">
              ...
              {selectedSpecialRequests.length > 0 && (
                <p className="text-sm">
                  ...
                  {selectedSpecialRequests
                    .map(
                      (id) => specialRequests.find((s) => s.id === id)?.label
                    )
                    .join(", ")}
                </p>
              )}
            ...
            >
              {step === 1 && (
                <div>
                  ...
                  <p className="mt-1 text-muted-foreground">
                    Choose what you'd like — each has individual pricing
                  </p>
                  <div className="mt-6 space-y-3">
                    {serviceOptions
                      .filter((s) => host.services.includes(s.key))
                      .map((s) => (
                        <button
                          key={s.key}
              ...
              {step === 3 && (
                <div>
                  ...
                  <p className="mt-1 text-muted-foreground">
                    Add extras to make your trip special (+$15 each)
                  </p>
                  <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {specialRequests.map((sr) => (
                      <button
                        key={sr.id}
                        onClick={() => toggleSpecialRequest(sr.id)}
              ...
              {step === 4 && (
                <div>
                  ...
                  <div className="mt-6 space-y-4">
                    <div>
                      <label className="text-sm font-medium text-foreground mb-2 block">
                        Message to {host.name} (optional)
                      </label>
                      <Textarea
                        value={message}
                        onChange={(e) => setMessage(e.target.value)}
                        placeholder={`Hi ${host.name}, I'm excited about...`}
                        rows={4}
                      />
                    </div>
                  </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground flex items-center gap-1">
                    <MapPin className="w-3 h-3" /> {host.city}
                  </p>
                </div>
              </div>
              ...
              <div className="border-t border-border pt-4 space-y-2">
                  ...
                  return (
                    <div key={s} className="flex justify-between text-sm">
                      <span className="text-muted-foreground">
                        {opt.icon} {opt.label} (${opt.price} × {days}d ×{" "}
                        {guests})
                      </span>
                      <span className="text-foreground">
                        ${opt.price * days * guests}
                      </span>
                    </div>
                  );
                })}
                {selectedSpecialRequests.length > 0 && (
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">
                      ✨ Special requests ({selectedSpecialRequests.length})
                    </span>
                    <span className="text-foreground">
                      ${specialRequestFee}
                    </span>
