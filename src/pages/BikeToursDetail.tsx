...
const bikeRoutes = [
  {
    ...
    maxRiders: 8,
    route:
      "Delhi → Manali → Rohtang → Keylong → Sarchu → Leh → Khardung La → Pangong → Leh",
    passes: [
      "Rohtang La (13,051 ft)",
      "Baralacha La (16,020 ft)",
      "Lachalung La (16,616 ft)",
      "Tanglang La (17,582 ft)",
  ...
  },
  {
    ...
    tagline: "Through Forts & Desert Dunes",
    image:
  ...
  },
  {
    ...
    tagline: "Beach Vibes on Two Wheels",
    image:
      ...
      <div className="relative h-[50vh] min-h-[350px] overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-10 mx-auto max-w-6xl">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs uppercase tracking-wider font-bold text-primary-foreground bg-primary/80 px-2 py-0.5 rounded-sm">
              Bike & Scooty Tours
            </span>
          </div>
          ...
          {bikeRoutes.map((r) => (
            <button
              ...
              className={`shrink-0 rounded-2xl overflow-hidden w-48 transition-all ${
                activeRoute.id === r.id
                  ? "ring-2 ring-primary shadow-elevated scale-105"
            ...
            >
              <div className="relative aspect-[16/10]">
                ...
                <div className="absolute bottom-2 left-2 right-2">
                  <p className="text-primary-foreground text-xs font-bold truncate">
                    {r.name}
                  </p>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                {
                  label: "Max Altitude",
                  value: activeRoute.elevation,
                  icon: Mountain,
                },
                {
                  label: "Max Riders",
                  value: `${activeRoute.maxRiders}`,
                  icon: Users,
                },
              ].map((s) => (
                <div
            ...
            <div className="rounded-2xl bg-card p-6 shadow-card">
              ...
              <div className="flex flex-wrap gap-2">
                {activeRoute.bikes.map((b) => (
                  <span
                    key={b}
                    className="text-sm bg-secondary text-foreground px-4 py-2 rounded-xl font-medium"
                  >
                    {b}
                  </span>
                ))}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {activeRoute.gallery.map((img, i) => (
                  <motion.div
                    ...
                    className="relative aspect-[4/3] rounded-xl overflow-hidden shadow-card group cursor-pointer"
                  >
                    <img
                      src={img}
                      alt=""
            ...
            <div className="flex flex-wrap gap-2">
              {activeRoute.tags.map((tag) => (
                <span
                  key={tag}
                  className="text-sm bg-primary/5 text-primary border border-primary/20 px-3 py-1.5 rounded-full font-medium"
                >
                  {tag}
                </span>
              ))}
            ...
            <div className="sticky top-24 space-y-4">
              <div className="rounded-2xl bg-card p-5 shadow-card">
                <div className="flex items-center gap-2 mb-3">
                  <span
                    className={`text-xs font-bold px-2.5 py-1 rounded-full ${
                      difficultyColor[activeRoute.difficulty]
                    }`}
                  >
                ...
                <p className="text-xs text-muted-foreground mb-4">
                  Best season: {activeRoute.bestSeason}
                </p>
                <Link to="/explore">
                ...
                <div className="flex gap-2 mt-3">
                  <Button
                    ...
                    onClick={() =>
                      setLiked((p) =>
                        p.includes(activeRoute.id)
                          ? p.filter((x) => x !== activeRoute.id)
                          : [...p, activeRoute.id]
                      )
                    }
                  >
              ...
              <div className="rounded-2xl bg-primary/5 border border-primary/20 p-5">
                ...
                <ul className="space-y-1.5 text-xs text-muted-foreground">
                  <li className="flex items-start gap-1.5">
                    ...
                    Certified mechanic on every trip
                  </li>
                  <li className="flex items-start gap-1.5">
                    <Shield className="w-3 h-3 text-accent mt-0.5 shrink-0" />{" "}
