...
import { destinations, experiences, hosts, reviews } from "@/lib/data";
...
const DestinationDetail = () => {
  ...
  const cityExperiences = experiences.filter(
    (e) => e.hostCity === destination.name
  );
  const cityReviews = reviews.filter((r) =>
    cityHosts.some((h) => h.id === r.hostId)
  );
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="relative h-[70vh] min-h-[500px] overflow-hidden">
        ...
        >
          <img
            src={heroImages[activeHeroImg % heroImages.length]}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-10 max-w-7xl mx-auto">
          ...
          >
            <Link
              to="/destinations"
              className="inline-flex items-center gap-1 text-primary-foreground/80 text-sm mb-4 hover:text-primary-foreground"
            >
            ...
            <div className="flex items-end justify-between gap-4">
              <div>
                ...
                <h1 className="text-5xl sm:text-7xl font-bold text-primary-foreground leading-none mt-1">
                  {destination.name}
                </h1>
                <p className="text-xl text-primary-foreground/90 mt-2 italic">
                  {destination.tagline}
                </p>
                <div className="mt-4 flex flex-wrap gap-3">
                  ...
                  <span className="flex items-center gap-1.5 text-sm text-primary-foreground/80 bg-primary-foreground/10 backdrop-blur-md px-3 py-1.5 rounded-full">
                    <Users className="w-3.5 h-3.5" /> {destination.hostCount}{" "}
                    Local Hosts
                  </span>
                  {destination.sites && (
                    <span className="flex items-center gap-1.5 text-sm text-primary-foreground/80 bg-primary-foreground/10 backdrop-blur-md px-3 py-1.5 rounded-full">
                      <Camera className="w-3.5 h-3.5" />{" "}
                      {destination.sites.length} Sites
                    </span>
                  )}
                </div>
              </div>
            ...
            <button
              ...
              className={`w-14 h-14 rounded-lg overflow-hidden border-2 transition-all ${
                i === activeHeroImg
                  ...
                  : "border-primary-foreground/30 opacity-70 hover:opacity-100"
              }`}
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          {destination.highlights && (
            <div className="mt-4 flex flex-wrap gap-2">
              {destination.highlights.map((h) => (
                <span
                  key={h}
                  className="text-sm bg-secondary text-foreground px-3 py-1.5 rounded-full font-medium"
                >
                  {h}
                </span>
              ))}
          ...
          >
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-sm text-muted-foreground">
                {destination.sites.length} places to explore
              </span>
            </div>
                    ...
                    >
                      ...
                      <div
                        className={`relative h-56 bg-gradient-to-br ${
                          siteGradients[destination.sites[selectedSite].type] ||
                      ...
                      >
                        <div className="absolute inset-0 flex items-center justify-center">
                          {(() => {
                            const Icon =
                              siteIcons[destination.sites[selectedSite].type] ||
                              Landmark;
                            return (
                      ...
                      <div className="p-6 -mt-8 relative">
                        <h3 className="text-2xl font-bold text-foreground">
                          {destination.sites[selectedSite].name}
                        </h3>
                        <p className="mt-2 text-muted-foreground leading-relaxed">
                          {destination.sites[selectedSite].description}
                        </p>
                        ...
                        <div className="mt-5">
                          <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2">
                            <Play className="w-4 h-4 text-primary" /> Traveler
                            Video Review
                          </h4>
                          <div className="relative aspect-video rounded-xl overflow-hidden bg-secondary group cursor-pointer">
                            <img
                              src={
                                heroImages[
                                  (selectedSite + 1) % heroImages.length
                                ]
                              }
                              alt="Video review"
                        ...
                        <div className="mt-4 flex gap-2">
                          ...
                          <VirtualTour
                            siteName={destination.sites[selectedSite].name}
                            siteType={destination.sites[selectedSite].type}
                          />
                        </div>
                      </div>
                    </motion.div>
                    ...
                    >
                      <div className="text-center p-8">
                        ...
                        <p className="text-sm text-muted-foreground/70 mt-1">
                          Click on any monument or site from the list
                        </p>
                      </div>
                    </motion.div>
                  )}
        ...
        >
          <h2 className="text-2xl font-bold text-foreground mb-6 flex items-center gap-2">
            <Play className="w-6 h-6 text-primary" /> Video Reviews from
            Travelers
          </h2>
        ...
        >
          ...
          <p className="mt-2 text-primary-foreground/80 max-w-xl mx-auto">
            Connect with verified local hosts for an authentic, safe, and
          ...
          <div className="mt-6 flex justify-center gap-3">
            ...
            <Link to="/experiences">
              <Button
                size="lg"
                variant="outline"
                className="rounded-full border-primary-foreground/30 text-primary-foreground hover:bg-primary-foreground/10 px-8"
              >
                Browse Experiences
              </Button>
            </Link>
