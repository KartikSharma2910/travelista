...
import { Check, Copy, Gift, Share2, Star, Users } from "lucide-react";
...
const Referrals = () => {
  ...
  useEffect(() => {
    ...
    const fetchReferrals = async () => {
      ...
      if (data) {
        setReferrals(data);
        setTotalPoints(
          data.reduce((sum, r) => sum + (r.reward_points || 0), 0)
        );
      }
  ...
  const shareLink = () => {
    ...
    if (navigator.share) {
      navigator.share({
        title: "Join Travelista!",
        text: "Use my referral link to get bonus rewards!",
        url,
      });
    } else {
      copyCode();
        ...
        >
          ...
          <div className="rounded-2xl bg-gradient-to-br from-primary/10 via-accent/5 to-secondary p-8 text-center">
            ...
            <p className="text-muted-foreground mt-2 max-w-lg mx-auto">
              Share your referral link. When friends sign up and book, you both
              earn reward points!
            </p>
          </div>
            ...
            {[
              {
                ...
                value: referrals.filter((r) => r.referred_id).length,
                icon: Users,
              },
