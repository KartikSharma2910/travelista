...
const tripTypeLabels: Record<string, string> = {
  bike_tour: "🏍️ Bike Tour",
  car_trip: "🚗 Car Trip",
  bus_trip: "🚌 Bus Trip",
  road_trip: "🛣️ Road Trip",
...
const tripFAQs = [
  {
    q: "How do I join this trip?",
    a: "Click 'Join This Trip' and submit a booking request. The trip organizer will confirm your spot within 24 hours.",
  },
  {
    q: "Can I cancel after joining?",
    a: "Yes, free cancellation up to 7 days before the start date. After that, a 50% refund applies.",
  },
  {
    q: "What should I bring?",
    a: "The organizer will share a packing list after confirmation. Essentials: ID proof, comfortable clothing, personal medication.",
  },
  {
    q: "Is travel insurance included?",
    a: "Basic platform insurance is included. We recommend getting additional travel insurance for adventure trips.",
  },
  {
    q: "What if the trip gets cancelled?",
    a: "Full refund if cancelled by the organizer. We also help find alternative trips.",
  },
];
...
const pastTrips = [
  {
    ...
    travelers: 8,
    rating: 4.9,
    highlight: "Amazing group, perfect weather",
  },
  {
    date: "Dec 2025",
    travelers: 6,
    rating: 4.8,
    highlight: "Snow-capped passes, unforgettable",
  },
  {
    date: "Oct 2025",
    travelers: 10,
    rating: 4.7,
    highlight: "Great camaraderie and food",
  },
];
...
const TripDetail = () => {
  ...
  const hostReviews = matchingHost
    ? reviews.filter((r) => r.hostId === matchingHost.id)
    : [];

  // Get destination highlights
  const destKey = Object.keys(highlightReels).find(
    (k) => trip.destination?.includes(k) || trip.title?.includes(k)
  );
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-20 px-4 sm:px-6 lg:px-8 mx-auto max-w-5xl pb-16">
        ...
        >
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            ...
            {isTrending && (
              <span className="text-xs font-bold px-3 py-1 rounded-full bg-destructive/10 text-destructive flex items-center gap-1">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
            <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="flex gap-3 overflow-x-auto pb-2 snap-x">
                    {reels.map((reel, i) => (
                      ...
                      >
                        ...
                        <p className="text-xs font-bold text-foreground text-center px-2 leading-tight">
                          {reel.name}
                        </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="rounded-2xl bg-card p-5 shadow-card">
                  <h3 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2">
                    <TripIcon className="w-4 h-4 text-primary" /> Vehicle &
                    Transport
                  </h3>
                  <div className="space-y-2 text-sm text-muted-foreground">
                    <p>
                    ...
                    {trip.includes_transport && (
                      <p className="flex items-center gap-1 text-accent">
                        <CheckCircle className="w-3 h-3" /> Transport included
                      </p>
                    )}
                    {trip.trip_type === "bike_tour" && (
                      <div className="mt-2 p-3 rounded-xl bg-secondary/50">
                        ...
                        <p className="text-xs">
                          Includes: Helmet, Riding Jacket, Saddlebags
                        </p>
                      </div>
                    )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  {
                    label: "Duration",
                    value: trip.duration || "Flexible",
                    icon: Clock,
                  },
                  {
                    label: "Max Travelers",
                    value: trip.max_travelers,
                    icon: Users,
                  },
                  {
                    label: "Joined",
                    value: `${joinedCount}/${trip.max_travelers || 10}`,
                    icon: Star,
                  },
                  {
                    label: "Price Model",
                    value: trip.price_model,
                    icon: Compass,
                  },
                ].map((s) => (
                  <div
                ...
                <div className="grid grid-cols-2 gap-3">
                  {trip.includes_food && (
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      <CheckCircle className="w-4 h-4 text-accent" /> Food &
                      Meals
                    </div>
                  )}
                  {trip.includes_stay && (
                  ...
                  {trip.includes_transport && (
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      <CheckCircle className="w-4 h-4 text-accent" /> Transport
                    </div>
                  )}
                  {trip.includes_activities && (
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      <CheckCircle className="w-4 h-4 text-accent" /> Activities
                    </div>
                  )}
                </div>
                ...
                <div className="rounded-2xl bg-card p-6 shadow-card">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {trip.highlights.map((h: string) => (
                      <span
                        key={h}
                        className="text-sm bg-primary/5 text-primary border border-primary/20 px-3 py-1.5 rounded-full"
                      >
                        {h}
                      </span>
                    ))}
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {pt.travelers} travelers joined
                        </p>
                      </div>
                      <div className="flex -space-x-1">
                        {Array.from({ length: Math.min(pt.travelers, 4) }).map(
                          (_, j) => (
                            <div
                              key={j}
                              className="w-6 h-6 rounded-full bg-primary/15 border border-background flex items-center justify-center text-[9px] font-bold text-primary"
                            >
              ...
              {hostReviews.length > 0 && (
                <div className="rounded-2xl bg-card p-6 shadow-card">
                  <h2 className="text-lg font-bold text-foreground mb-4 flex items-center gap-2">
                    <Star className="w-5 h-5 text-primary" /> Reviews from Past
                    Travelers
                  </h2>
                  <div className="space-y-3">
                ...
                <div className="rounded-2xl bg-card p-5 shadow-card">
                  <div className="flex items-baseline gap-1 mb-2">
                    <span className="text-3xl font-bold text-foreground">
                      {format(trip.total_price || 0)}
                    </span>
                    <span className="text-sm text-muted-foreground">
                      {trip.price_model === "per_person"
                        ? "/person"
                        : trip.price_model === "split"
                        ? " (split)"
                        : " total"}
                    </span>
                  </div>
                  ...
                  {user ? (
                    <Button
                      ...
                      size="lg"
                      onClick={() => toast({ title: "Booking request sent!" })}
                  ...
                  <div className="flex gap-2 mt-3">
                    <Button
                      variant="outline"
                      className="flex-1 rounded-full gap-1 text-sm"
                    >
                  ...
                  <div className="rounded-2xl bg-card p-5 shadow-card">
                    ...
                    <Link to={`/host/${matchingHost.id}`} className="group">
                      <div className="flex items-center gap-3">
                        <img
                          src={matchingHost.image}
                          alt={matchingHost.name}
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {matchingHost.city} · ⭐ {matchingHost.rating}
                          </p>
                          <p className="text-xs text-muted-foreground italic">
                            "{matchingHost.tagline}"
                          </p>
                        </div>
                      </div>
                    </Link>
                    ...
                    {matchingHost.introVideoUrl && (
                      <Button
                        variant="outline"
                        size="sm"
                        className="w-full mt-3 rounded-full gap-1 text-xs"
                ...
                <div className="rounded-2xl bg-primary/5 border border-primary/20 p-5">
                  ...
                  <ul className="space-y-1.5 text-xs text-muted-foreground">
                    <li className="flex items-start gap-1.5">
                      <Shield className="w-3 h-3 text-accent mt-0.5" /> Verified
                      trip organizer
                    </li>
                    <li className="flex items-start gap-1.5">
                      <Shield className="w-3 h-3 text-accent mt-0.5" /> Secure
                      payment via platform
                    </li>
                    <li className="flex items-start gap-1.5">
                      <Shield className="w-3 h-3 text-accent mt-0.5" /> 24/7
                      support during trip
                    </li>
                    <li className="flex items-start gap-1.5">
                      <Shield className="w-3 h-3 text-accent mt-0.5" /> Free
      ...
      <VideoModal
        ...
        videoUrl={matchingHost?.introVideoUrl}
        title={`${matchingHost?.name}'s Intro`}
      />
    </div>
  );
};
