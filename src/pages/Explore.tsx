...
import { Award, Calendar, Filter, MapPin, Search, Tag, X } from "lucide-react";
  ...
  const filteredHosts = useMemo(() => {
    return hosts.filter((h) => {
      ...
      const matchesExpertise =
        selectedExpertise.length === 0 ||
        selectedExpertise.some((e) => h.expertiseTags?.includes(e));
      const matchesTags =
        selectedTags.length === 0 ||
        selectedTags.some((t) => h.expertiseTags?.includes(t));
      const matchesVibe =
        !activeVibe ||
        h.specialties?.includes(activeVibe) ||
        h.expertiseTags?.some((t) =>
          t.toLowerCase().includes(activeVibe!.toLowerCase())
        );
      return (
        matchesSearch &&
        matchesCity &&
      ...
      <div className="pt-24 pb-16 px-4 sm:px-6 lg:px-8 mx-auto max-w-7xl">
        ...
        >
          ...
          <p className="mt-2 text-muted-foreground">
            Find verified local companions across India
          </p>
        </motion.div>
          ...
          {showFilters && (
            ...
            >
              <div className="mt-4 rounded-2xl bg-card shadow-card p-6 space-y-5 border border-border">
                ...
                <div>
                  <h4 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2">
                    <Calendar className="w-4 h-4 text-primary" /> Available
                    Month
                  </h4>
                  <div className="flex flex-wrap gap-2">
                    {months.map((m) => (
                ...
                <div>
                  <h4 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2">
                    <Tag className="w-4 h-4 text-primary" /> Explorer & Travel
                    Tags
                  </h4>
                  <div className="flex flex-wrap gap-2">
                    {specialTags.map((t) => (
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {vibeCategories.map((v) => (
            <button
              key={v.label}
              onClick={() =>
                setActiveVibe(activeVibe === v.label ? null : v.label)
              }
