...
import { ArrowLeft, Camera, MapPin, Send, Video } from "lucide-react";
...
const BetaWandererApply = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.fullName || !form.email || !form.city) {
      toast({
        title: "Name, email, and city are required",
        variant: "destructive",
      });
      return;
    }
    ...
    const { error } = await supabase.from("beta_wanderers").insert({
      ...
      city: form.city,
      bio: form.bio || null,
      travel_styles: form.travelStyles,
          ...
          >
            ...
            <div className="flex gap-3 justify-center mt-6">
              <Link to="/beta-wanderers">
                <Button className="rounded-full">View All Wanderers</Button>
              </Link>
              <Link to="/">
        ...
        >
          ...
          <div className="rounded-2xl bg-gradient-to-br from-primary/10 via-accent/5 to-secondary p-8 text-center mb-8">
            ...
            <p className="text-muted-foreground mt-2 max-w-lg mx-auto">
              ...
              and earn rewards. Be the eyes and ears of the Travelista
              community!
            </p>
                ...
                {travelStyleOptions.map((s) => (
                  <button
                    ...
                    onClick={() =>
                      setForm((p) => ({
                        ...p,
                        travelStyles: toggleArray(p.travelStyles, s),
                      }))
                    }
                    className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${
                      form.travelStyles.includes(s)
                ...
                {destinationOptions.map((d) => (
                  <button
                    ...
                    onClick={() =>
                      setForm((p) => ({
                        ...p,
                        preferredDestinations: toggleArray(
                          p.preferredDestinations,
                          d
                        ),
                      }))
                    }
                    className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${
                      form.preferredDestinations.includes(d)
            ...
            <div className="rounded-2xl bg-card p-6 shadow-card space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder="Link to a travel video you've made"
                />
              </div>
            </div>
