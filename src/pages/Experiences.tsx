...
const allCategories = [...vibeCategories, { label: "Bike Tour", emoji: "🏍️" }];
  ...
  const filtered = useMemo(() => {
    let result = allExperiences.filter((e) => {
      ...
      const matchesSearch =
        ...
        e.hostCity.toLowerCase().includes(q) ||
        e.category.toLowerCase().includes(q);
      const matchesCategory = !activeCategory || e.category === activeCategory;
      const matchesLocation =
        !selectedLocation || e.hostCity === selectedLocation;
      const matchesDifficulty =
        !selectedDifficulty || e.difficulty === selectedDifficulty;
      const matchesPrice =
        !selectedPrice ||
        (e.price >= selectedPrice.min && e.price < selectedPrice.max);
      return (
        matchesSearch &&
        matchesCategory &&
        matchesLocation &&
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="relative pt-20 pb-8 px-4 sm:px-6 lg:px-8 mx-auto max-w-7xl">
        ...
        >
          ...
          <p className="mt-3 text-lg text-muted-foreground">
            Curated adventures, cultural immersions & bike tours across India
          </p>
        </motion.div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {priceRanges.map((p) => (
                        <button
                          ...
                          onClick={() =>
                            setSelectedPrice(
                              selectedPrice?.label === p.label ? null : p
                            )
                          }
                          className={`text-xs px-2.5 py-1 rounded-full transition-all ${
        ...
        <div className="mt-6 flex flex-wrap gap-2 justify-center max-w-4xl mx-auto">
          {allCategories.map((v) => (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() =>
                setActiveCategory(activeCategory === v.label ? null : v.label)
              }
              className={`flex items-center gap-1.5 rounded-full px-4 py-2 text-sm font-medium transition-all ${
                activeCategory === v.label
      ...
      <div className="px-4 sm:px-6 lg:px-8 mx-auto max-w-7xl pb-16">
        ...
        {(!activeCategory || activeCategory === "Bike Tour") &&
          !searchQuery && (
            <motion.section
              initial={{ opacity: 0, y: 20 }}
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Ride through India's most scenic routes
                  </p>
                </div>
              </div>
                    ...
                    >
                      <div className="relative aspect-[4/3] overflow-hidden">
                          ...
                          {exp.difficulty && (
                            <span
                              className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${
                                (exp.difficulty as string) === "Extreme"
                        ...
                        <div className="absolute bottom-3 left-3 right-3">
                          <p className="text-primary-foreground font-bold text-sm line-clamp-2">
                            {exp.title}
                          </p>
                          <p className="text-primary-foreground/70 text-xs mt-0.5">
                            {exp.hostName} • {exp.hostCity}
                          </p>
                        </div>
                      </div>
                      <div className="p-4">
                        ...
                        {!exp.isYearRound && exp.validFrom && (
                          <div className="mt-2 flex items-center gap-1 text-[10px] text-muted-foreground bg-secondary px-2 py-1 rounded-full w-fit">
                            <Calendar className="w-3 h-3" /> {exp.validFrom} –{" "}
                            {exp.validTo}
                          </div>
                        )}
                        {exp.isYearRound && (
            ...
            {allExperiences.slice(0, 4).map((exp, i) => (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3">
                  ...
                  <p className="text-sm font-semibold text-primary-foreground line-clamp-2">
                    {exp.title}
                  </p>
                </div>
              </motion.div>
