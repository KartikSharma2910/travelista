...
const siteColors: Record<string, string> = {
  monument: "bg-primary/10 text-primary",
  temple: "bg-accent/10 text-accent",
  palace: "bg-primary/10 text-primary",
  fort: "bg-destructive/10 text-destructive",
  nature: "bg-accent/10 text-accent",
  beach: "bg-accent/10 text-accent",
  market: "bg-primary/10 text-primary",
  museum: "bg-secondary text-muted-foreground",
};
...
const Destinations = () => {
  ...
  const allTags = [
    ...new Set(destinations.flatMap((d) => d.experienceTags || [])),
  ];
      ...
      <div className="pt-24 pb-16 px-4 sm:px-6 lg:px-8 mx-auto max-w-7xl">
        ...
        >
          ...
          <p className="mt-3 text-lg text-muted-foreground max-w-2xl mx-auto">
            Pin-drop into India's most extraordinary cities. Discover monuments,
            temples, palaces, and hidden gems with local hosts.
          </p>
        </motion.div>
            ...
            {filteredDestinations.map((d, i) => {
              ...
              return (
                <motion.button
                  ...
                  onClick={() =>
                    setExpandedCity(expandedCity === d.name ? null : d.name)
                  }
                  className={`relative group rounded-xl p-4 text-center transition-all duration-300 hover:shadow-card-hover cursor-pointer ${
                    expandedCity === d.name
          ...
          {filteredDestinations.map((d, i) => {
            ...
            return (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: i * 0.03 }}
                className={`rounded-2xl bg-card shadow-card transition-all duration-300 overflow-hidden ${
                  isExpanded
                    ? "shadow-elevated ring-2 ring-primary/20"
              ...
              >
                ...
                <div
                  ...
                  onClick={() => setExpandedCity(isExpanded ? null : d.name)}
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <Link
                          to={`/destination/${d.name.toLowerCase()}`}
                          className="text-sm font-medium text-primary ml-2 hover:underline"
                        >
                          {d.tagline}
                        </Link>
                      </div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-3">
                        <span className="flex items-center gap-1 text-xs text-muted-foreground">
                          ...
                          {cityHosts.length > 0
                            ? `${cityHosts.length} hosts available`
                            : `${d.hostCount} hosts`}
                        </span>
                        {d.bestSeason && (
                          <span className="flex items-center gap-1 text-xs text-muted-foreground">
                        ...
                        {d.sites && (
                          <span className="flex items-center gap-1 text-xs text-muted-foreground">
                            <Camera className="w-3 h-3" /> {d.sites.length}{" "}
                            sites
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex flex-col items-end gap-3 shrink-0">
                      {cityHosts.length > 0 && (
                        <div className="flex -space-x-2">
                          ...
                          {cityHosts.length > 4 && (
                            <div className="w-8 h-8 rounded-full bg-primary/10 border-2 border-card flex items-center justify-center text-[10px] font-bold text-primary">
                              +{cityHosts.length - 4}
                            </div>
                          )}
                        </div>
                    ...
                    >
                      <div className="px-6 pb-6 border-t border-border pt-4">
                        ...
                        {d.sites && d.sites.length > 0 && (
                          <div>
                            <h4 className="font-bold text-foreground mb-4 flex items-center gap-2">
                              ...
                              Sites & Monuments to Explore
                            </h4>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                              {d.sites.map((site) => {
                                const Icon = siteIcons[site.type] || Landmark;
                                const isSelected =
                                  selectedSite === `${d.name}-${site.name}`;
                                return (
                                  <motion.div
                                    key={site.name}
                        ...
                        {cityHosts.length > 0 && (
                          <div className="mt-6">
                            <h4 className="font-bold text-foreground mb-4 flex items-center gap-2">
                              <Users className="w-4 h-4 text-primary" /> Meet
                              Your Local Hosts in {d.name}
                            </h4>
