...
const demoWanderers = [
  {
    ...
    city: "Delhi",
    bio: "Solo traveler & vlogger exploring offbeat trails across Himalayas.",
  ...
  },
  {
    ...
    city: "Bangalore",
    bio: "Cultural photographer documenting India's living heritage.",
  ...
  },
  {
    ...
    city: "Mumbai",
    bio: "Foodie traveler on a mission to taste every state in India.",
  ...
  },
  {
    ...
    city: "Kochi",
    bio: "Backwater explorer & ayurveda enthusiast sharing Kerala secrets.",
...
const BetaWanderers = () => {
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        if (data && data.length > 0) setWanderers([...demoWanderers, ...data]);
      });
  }, []);

  const filtered = wanderers.filter(
    (w) =>
      w.full_name.toLowerCase().includes(search.toLowerCase()) ||
      w.city.toLowerCase().includes(search.toLowerCase()) ||
      w.travel_styles?.some((s: string) =>
        ...
        >
          ...
          <div className="rounded-2xl bg-gradient-to-br from-primary/10 via-accent/5 to-secondary p-8 sm:p-12 text-center mb-8">
            ...
            <p className="text-muted-foreground mt-2 max-w-xl mx-auto">
              Our community explorers travel to new places, create content, and
              share authentic feedback to help fellow travelers.
            </p>
              ...
              {[
                {
                  icon: Camera,
                  label: "Video Reviews",
                  value: `${wanderers.reduce(
                    (s, w) => s + (w.total_videos || 0),
                    0
                  )}+`,
                },
                ...
                <div className="p-5">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                            badgeColors[w.badge] || badgeColors.explorer
                          }`}
                        >
                  ...
                  <div className="flex flex-wrap gap-1 mt-3">
                    {w.travel_styles?.slice(0, 3).map((s: string) => (
                      <span
                        key={s}
                        className="text-[10px] bg-secondary text-foreground px-2 py-0.5 rounded-full"
                      >
                        {s}
                      </span>
                    ))}
                  ...
                  <Link to={`/beta-wanderer/${w.id}`}>
                    <Button
                      variant="outline"
                      size="sm"
                      className="w-full mt-4 rounded-full text-xs gap-1"
                    >
