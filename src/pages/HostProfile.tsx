...
const serviceColors: Record<string, string> = {
  Guide: "from-primary/20 to-primary/5",
  Stay: "from-accent/20 to-accent/5",
  Transport: "from-destructive/10 to-destructive/5",
  Food: "from-primary/15 to-primary/5",
};
...
const HostProfile = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "stay" | "transport" | "food" | "experiences" | "reviews"
  >("overview");
  ...
  const setRoomSlider = (roomName: string, idx: number) =>
    setRoomSliderIndex((prev) => ({ ...prev, [roomName]: idx }));
  ...
  const tabs = [
    ...
    ...(host.stayInfo
      ? [{ id: "stay" as const, label: "Stay", icon: Home }]
      : []),
    ...(host.transportInfo
        ...
        <div className="flex flex-wrap gap-3 mb-8">
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (navigator.share) {
                navigator.share({
                  title: `${host.name} on Travelista`,
                  text: `Check out ${host.name}'s hosting in ${host.city}!`,
                  url,
                });
              } else {
            ...
            return (
              <motion.div
                ...
                className={`rounded-2xl bg-gradient-to-br ${
                  serviceColors[s] || "from-secondary to-muted"
                } p-5 text-center cursor-pointer transition-shadow hover:shadow-card`}
                onClick={() => {
                  if (s === "Stay" && host.stayInfo) setActiveTab("stay");
                  if (s === "Transport" && host.transportInfo)
          ...
          <div className="flex gap-3 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide">
            {reels.map((reel, i) => (
              <motion.div
                ...
                className={`relative shrink-0 w-36 sm:w-44 aspect-[9/16] rounded-2xl overflow-hidden cursor-pointer snap-start transition-all duration-300 ${
                  activeReel === i
              ...
              >
                ...
                <div className="absolute bottom-2 left-2 right-2">
                  <p className="text-primary-foreground text-xs font-semibold leading-tight truncate">
                    {reel.title}
                  </p>
            ...
            {tabs.map((t) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium whitespace-nowrap rounded-full transition-all ${
                    activeTab === t.id
                    ...
                    {host.expertiseTags && (
                      <div className="mt-3 flex flex-wrap gap-2">
                        {host.expertiseTags.map((t) => (
                          <span
                            key={t}
                            className="text-xs bg-accent/10 text-accent px-3 py-1 rounded-full font-medium flex items-center gap-1"
                          >
                    ...
                    >
                      ...
                      {room.images && room.images.length > 0 ? (
                        <div className="relative aspect-video overflow-hidden group">
                            ...
                            <>
                              <button
                                onClick={(e) => {
                                  ...
                                  setRoomSlider(
                                    room.name,
                                    getRoomSlider(room.name) === 0
                                      ? room.images!.length - 1
                                      : getRoomSlider(room.name) - 1
                                  );
                                }}
                              ...
                              <span
                                className="absolute bottom-2 right-2 bg-foreground/60 backdrop-blur-sm text-primary-foreground text-[10px] font-medium px-2 py-0.5 rounded cursor-pointer"
                                onClick={() =>
                                  openLightbox(
                                    room.images!,
                                    getRoomSlider(room.name)
                          ...
                          <span className="absolute top-2 left-2 text-xs bg-primary text-primary-foreground px-2 py-0.5 rounded-full font-bold">
                            {room.type}
                          </span>
                        </div>
                      ) : (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="rounded-2xl bg-secondary p-5">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Check-in: {host.stayInfo.checkIn}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      Check-out: {host.stayInfo.checkOut}
                    </p>
                  </div>
                  <div className="rounded-2xl bg-secondary p-5">
                    <h4 className="text-sm font-bold text-foreground uppercase tracking-wider mb-2">
            ...
            {activeTab === "transport" && host.transportInfo && (
              <div className="space-y-6">
                <div className="rounded-2xl bg-card shadow-card p-6">
                  ...
                  <p className="text-muted-foreground leading-relaxed">
                    {host.transportInfo.description}
                  </p>
                </div>
                      ...
                      {v.image ? (
                        <div className="relative aspect-video overflow-hidden">
                          <img
                            ...
                            className="w-full h-full object-cover"
                          />
                          <span className="absolute top-2 left-2 text-xs bg-primary text-primary-foreground px-2.5 py-1 rounded-full font-bold">
                            {v.type}
                          </span>
                        </div>
                      ) : (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="rounded-2xl bg-secondary p-5">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {host.transportInfo.driverLanguages.join(", ")}
                    </p>
                  </div>
                  <div className="rounded-2xl bg-secondary p-5">
                    <h4 className="text-xs font-bold text-foreground uppercase tracking-wider mb-2">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {host.transportInfo.coverage.map((c) => (
                        <span
                          key={c}
                          className="text-xs bg-card text-muted-foreground px-2 py-0.5 rounded-full"
                        >
                          {c}
                        </span>
                      ))}
              ...
              <div className="space-y-6">
                  ...
                  <div className="rounded-2xl bg-secondary p-5">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {host.foodInfo.dietaryOptions.map((d) => (
                        <span
                          key={d}
                          className="text-xs bg-card border border-border text-muted-foreground px-2 py-0.5 rounded-full"
                        >
                          {d}
                        </span>
                      ))}
                ...
                <div className="flex flex-wrap gap-2">
                  {host.foodInfo.specialties.map((s) => (
                    <span
                      key={s}
                      className="text-sm bg-primary/5 text-foreground border border-primary/20 px-3 py-1.5 rounded-full"
                    >
                      ⭐ {s}
                    </span>
                  ))}
                ...
                {(host.foodInfo.minimumOrder ||
                  host.foodInfo.advanceNotice) && (
                  <div className="rounded-2xl bg-primary/5 border border-primary/20 p-5">
                    ...
                    <div className="space-y-1 text-sm text-muted-foreground">
                      {host.foodInfo.minimumOrder && (
                        <p>Minimum: {host.foodInfo.minimumOrder} persons</p>
                      )}
                      {host.foodInfo.advanceNotice && (
                        <p>Advance notice: {host.foodInfo.advanceNotice}</p>
                      )}
                    </div>
                  </div>
                )}
                ...
                <div className="rounded-2xl bg-card shadow-card p-6 mb-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 space-y-1">
                      {[5, 4, 3, 2, 1].map((n) => {
                        const count = hostReviews.filter(
                          (r) => r.rating === n
                        ).length;
                        const pct =
                          hostReviews.length > 0
                            ? (count / hostReviews.length) * 100
                            : 0;
                        return (
                          <div
                            key={n}
