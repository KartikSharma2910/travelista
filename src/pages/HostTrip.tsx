...
const HostTrip = () => {
        ...
        <div className="pt-24 pb-16 px-4 max-w-2xl mx-auto text-center">
          ...
          >
            ...
            <p className="mt-2 text-muted-foreground">
              Sign in to create and share trips with fellow travelers
            </p>
            <Link to="/signup">
  ...
  const addInclusion = () =>
    setForm((p) => ({ ...p, inclusions: [...p.inclusions, ""] }));
  const updateInclusion = (i: number, v: string) =>
    setForm((p) => ({
      ...p,
      inclusions: p.inclusions.map((h, j) => (j === i ? v : h)),
    }));
  ...
  const handleSubmit = async () => {
    ...
    if (error) {
      ...
      toast({
        title: "Trip listed! 🎉",
        description: "Your trip is now live for other travelers to join.",
      });
      ...
      <div className="pt-24 pb-16 px-4 sm:px-6 lg:px-8 mx-auto max-w-3xl">
        ...
        >
          <div className="text-center mb-8">
            ...
            <p className="mt-2 text-muted-foreground">
              Create a group trip for fellow travelers to join
            </p>
          </div>
          ...
          >
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="e.g., Ladakh Bike Expedition 2026"
                  />
                </div>
                <div>
                  <label className="text-sm font-medium text-foreground mb-2 block">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {tripTypes.map((t) => (
                      <button
                        key={t.id}
                        onClick={() => updateForm("tripType", t.id)}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {tripNatures.map((n) => (
                      <button
                        key={n.id}
                        onClick={() => updateForm("nature", n.id)}
                        className={`flex items-center gap-2 p-3 rounded-xl border transition-all text-left ${
                          form.nature === n.id
                            ? "border-primary bg-primary/5 shadow-card"
                            : "border-border hover:border-primary/30"
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="e.g., Delhi → Manali → Leh → Pangong → Delhi"
                  />
                </div>
                <div>
                  <label className="text-sm font-medium text-foreground mb-2 block">
                  ...
                  <div className="flex gap-2">
                    {[
                      ...
                      <button
                        key={d.id}
                        onClick={() => updateForm("tripDirection", d.id)}
                        className={`flex-1 p-3 rounded-xl border text-sm font-medium transition-all ${
                          form.tripDirection === d.id
                  ...
                  <div className="flex items-center gap-3">
                    <Button
                      ...
                      onClick={() =>
                        updateForm(
                          "maxTravelers",
                          Math.max(2, form.maxTravelers - 1)
                        )
                      }
                    >
                      −
                    ...
                    <Button
                      ...
                      onClick={() =>
                        updateForm(
                          "maxTravelers",
                          Math.min(50, form.maxTravelers + 1)
                        )
                      }
                    >
                      +
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {inclusionOptions.map((inc) => {
                    const key = `includes${
                      inc.id.charAt(0).toUpperCase() + inc.id.slice(1)
                    }` as keyof typeof form;
                    const isSelected = form[key] as boolean;
                    return (
                      <button
                        key={inc.id}
                        onClick={() => updateForm(key, !isSelected)}
                ...
                <div>
                  ...
                  {form.inclusions.map((inc, i) => (
                    <Input
                      ...
                      placeholder={`e.g., Breakfast, Tent, Guide`}
                      className="mb-2"
                    />
                  ))}
                  <Button
            ...
            {step === 4 && (
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {[
                      ...
                      <button
                        key={p.id}
                        onClick={() => updateForm("priceModel", p.id)}
                        className={`flex-1 p-3 rounded-xl border text-sm font-medium transition-all ${
                          form.priceModel === p.id
                            ? "border-primary bg-primary/5"
                            : "border-border"
                        }`}
                      >
                ...
                <div>
                  <label className="text-sm font-medium text-foreground mb-2 block">
                    {form.priceModel === "per_person"
                      ? "Price Per Person ($)"
                      : form.priceModel === "split"
                      ? "Total Trip Cost to Split ($)"
                      : "Total Price ($)"}
                  </label>
                  <Input
                    type="number"
                    ...
                    onChange={(e) =>
                      updateForm("totalPrice", Number(e.target.value))
                    }
                    placeholder="0"
                  />
                </div>
                ...
                <div className="rounded-xl bg-primary/5 border border-primary/20 p-5 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <span className="text-foreground">
                      {tripTypes.find((t) => t.id === form.tripType)?.label ||
                        "—"}
                    </span>
                    <span className="text-muted-foreground">Nature:</span>
                    <span className="text-foreground">
                      {tripNatures.find((n) => n.id === form.nature)?.label ||
                        "—"}
                    </span>
                    ...
                    <span className="text-foreground">{form.maxTravelers}</span>
